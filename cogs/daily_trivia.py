...
class DailyTrivia(commands.Cog):
    ...
    async def reveal_trivia_answer(self, answer_data: dict):
        ...
        if total_players > 0:
            percent_correct = (len(answer_data.get("winners", [])) / total_players) * 100
    ...
    async def post_trivia_question(self, guild_id: int, cfg: dict):
        ...
        if cfg.get("question_cache"):
            ...
            if results: question_data = results[0]
        ...
        try:
            ...
            async with self.config_lock:
                cfg["asked_questions"].append(question_data["question"])
                if len(cfg["asked_questions"]) > 200: cfg["asked_questions"] = cfg["asked_questions"][-200:]
                reveal_time = datetime.now(timezone.utc) + timedelta(minutes=cfg["reveal_delay"])
                cfg["pending_answers"].append({
                ...
                if "last_posted_date" in cfg:
                     cfg["last_posted_date"] = datetime.now(pytz.timezone(cfg["timezone"])).strftime("%Y-%m-%d")
                await self.save()
            return msg
            ...
            for guild_id_str, cfg in self.config.items():
                if pending_answers := cfg.get("pending_answers", []):
                    ...
                    for ans in pending_answers:
                        if now_utc >= datetime.fromisoformat(ans["reveal_at_iso"]):
    ...
    async def trivia_toggle(self, interaction: discord.Interaction, enabled: bool):
        async with self.config_lock:
            ...
            cfg["enabled"] = enabled
            await self.save()
        embed = discord.Embed(description=f"✅ Daily trivia is now **{'Enabled' if enabled else 'Disabled'}**.", color=EMBED_COLOR_TRIVIA)
        embed.set_footer(text=self.get_footer_text())
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def trivia_skip(self, interaction: discord.Interaction):
        ...
        async with self.config_lock:
            ...
            active_question = next((q for q in pending if q['channel_id'] == cfg['channel_id']), None)
